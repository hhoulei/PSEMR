...
#' @return list
#' @rdname Nonordered_Egger
#' @export

Nonordered_Egger <- function(betaYG, sebetaYG, betaXG, sebetaXG, betaMG, sebetaMG) {
    ...
    # direct effect
    reg1 <- lm(data1[, 1] ~ ., weights = se_data1[, 1]^-2, data = data1[, -1])
    reg1_result <- data.frame(coef = reg1$coef, se = summary(reg1)$coef[, 2]/min(summary(reg1)$sigma, 1), 
        pvalue = summary(reg1)$coef[, 4])
    reg1_result$low <- reg1_result[, 1] - qt(0.975, df = length(betaXG) - ncol(data1)) * reg1_result[, 2]
    reg1_result$up <- reg1_result[, 1] + qt(0.975, df = length(betaXG) - ncol(data1)) * reg1_result[, 2]
    
    p_test <- rbind(p_test, reg1_result[1, ])
    result[[1]] <- as.matrix(reg1_result[-1, ])
    
    for (h in 1:ncol(betaMG)) {
        reg1 <- lm(data1[, 1 + h] ~ data1[, ncol(data1)], weights = se_data1[, 1]^-2, data = data1)
        reg1_result <- data.frame(coef = reg1$coef, se = summary(reg1)$coef[, 2]/min(summary(reg1)$sigma, 
            1), pvalue = summary(reg1)$coef[, 4])
        reg1_result$low <- reg1_result[, 1] - qt(0.975, df = length(betaXG) - 2) * reg1_result[, 2]
        reg1_result$up <- reg1_result[, 1] + qt(0.975, df = length(betaXG) - 2) * reg1_result[, 2]
        
        p_test <- rbind(p_test, reg1_result[1, ])
        
        result[[(h + 1)]] <- as.matrix(reg1_result[-1, ])
    }
