...
Ordered_IVW <- function(betaYG, sebetaYG, betaXG, sebetaXG, betaMG, sebetaMG) {
    ...
    while (ncol(data1) > 2) {
        ...
        data2 <- data1[, -1]
        reg1 <- lm(data1[, 1] ~ . - 1, weights = se_data1[, 1]^-2, data = data2)
        ...
        result[[h]] <- as.matrix(reg1_result)
        h = h + 1
        data1 <- data1[, -1]
        se_data1 <- se_data1[, -1]
    }
    reg2 <- lm(data1[, 1] ~ data1[, 2] - 1, weights = se_data1[, 1]^-2)
    ...
    result[[h]] <- as.matrix(reg2_result)
